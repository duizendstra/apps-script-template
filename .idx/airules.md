...
- architecture_overview: |
  ...
  - **`gas/`:** Contains all source code pushed to Google Apps Script (this is the `rootDir` for clasp).
    ...
    - **`managers/`:** Directory containing modular server-side JavaScript (`.js`) files, each responsible for interacting with a specific Google service or domain (e.g., `gasCalendarManager.js`, `gasEmailManager.js`, `gasEventManager.js`, `gasGroupManager.js`, `gasProcesmanager.js`, `gasSheetManager.js`).
...
- key_patterns: |
  ...
  - **Server-Side Logic:** Functions written in `.js` files (within `gas/`) run on Google's servers (V8 Runtime). Uses global Google service objects like `SpreadsheetApp`, `CalendarApp`, `Utilities`, `PropertiesService`, `CacheService`, `AdminDirectory`.
  - **Modularity:** Manager pattern (`gas/managers/*.js`) used to isolate interactions with specific Google APIs or concerns. `gas/gasProcesmanager.js` likely coordinates calls between managers, often triggered by `gas/webapp.js` or time-based triggers.
  ...
  - **Error Handling:** Server-side uses `try...catch` blocks. Client-side uses `.withFailureHandler()` in `google.script.run` calls. `console.log` and `console.error` (or `Logger.log`) used for debugging on both client and server (viewable in GAS editor logs or browser console). Admin email notifications configured in `gas/config.js` might be used.
  - **Coding Standards:** Modern JavaScript (`const`/`let`, arrow functions, template literals), descriptive names, JSDoc comments (for functions, params, returns), modular functions.
...
- " - Generate idiomatic Google Apps Script code using the V8 runtime (modern JavaScript features like `const`, `let`, arrow functions, template literals)."
- " - Adhere to the project's Coding Standards (Modern JS, Descriptive Naming, JSDoc, Error Handling, Logging, Modularity, JSON preference)."
...
- " - Utilize the appropriate global GAS services (`SpreadsheetApp`, `CalendarApp`, `MailApp`, `Utilities`, `PropertiesService`, `CacheService`, `AdminDirectory`, etc.). Refer to official GAS documentation for API usage."
- " - Implement `try...catch` blocks for server-side operations that might fail (especially API calls, file access)."
- " - Use `console.log` or `Logger.log` for server-side debugging, and `console.log`/`console.error` for client-side debugging."
- " - Respect the modular structure (`gas/webapp.js` for UI dispatch, `gas/managers/*.js` for specific service interactions, `gas/gasProcesmanager.js` for orchestration). Place new functions in the appropriate file."
...
- " - Be mindful of GAS quotas and limitations (execution time, API calls per day, etc.). Suggest efficient patterns where applicable (e.g., batch operations for Sheets/Docs)."
